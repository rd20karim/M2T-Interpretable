...
class dataset_class(Dataset):
    def __init__(self, path, train=False, test=False, val=False, path_txt=None, min_freq=1, filter_data=False,
                 joint_angles=False, multiple_references=False):
        ...
        self.sentences =  data["descriptions"]  # flat all descriptions in order
        ...
        def remove_samples(data,p=100):
            return (data[0][[(arr.shape[0] <= p) for arr in data[0]]],
                    data[1][[(arr.shape[0] <= p) for arr in data[0]]])

        if not multiple_references:
            [self.X_train, self.y_train], [self.X_val, self.y_val], [self.X_test, self.y_test] = [remove_samples(data) for data in [[self.X_train, self.y_train],
